...
var jurassicPark = function() {
    ...
    var generateStraydinasour = function() {
        var StrayInt = Math.floor(Math.random() * ((dinasours.length - 1) - 0 + 1)) + 0;
...
this.dinasourAction = function() {
    if(self.userIsActive === true && self.dinasourHp > 0) {
        var dinasourAttackPower = Math.floor(Math.random() * (50 - 25 + 1) + 25);
...
this.initialize = function() {
        self.dinasourIsActive = true;
        const walking = readlineSync.keyIn('Press w to Walk: ');
        console.log (walking);
        if (walking === "w") {
